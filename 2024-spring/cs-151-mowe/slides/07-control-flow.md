...
- take 1 or more boolean values / expressions and produces a single boolean value as output
- `&&` - and operator
  - takes 2 values and returns `true` only if both values are `true`
- `||` - or operator
  - takes 2 values and returns `true` if at least one value is `true`
